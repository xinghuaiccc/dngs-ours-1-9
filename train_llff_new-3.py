                    ...
                    if iteration % 500 == 0:
                        ...
                        if xyz.shape[0] > k:
                            ...
                            for start in range(0, xyz.shape[0], chunk_size):
                                ...
                                dists = torch.cdist(xyz[start:end].float(), xyz.float())
                                knn = torch.topk(dists, k + 1, largest=False).values
                                mean_k = knn[:, 1:].mean(dim=1)
                                mean_k_all.append(mean_k)
                            mean_k_all = torch.cat(mean_k_all, dim=0)
                            mean_val = mean_k_all.mean()
                            std_val = mean_k_all.std(unbiased=False)
                            thresh = mean_val + 3.0 * std_val
                            prune_mask = (mean_k_all > thresh) & (opacities < 0.5)
                            if prune_mask.any():
